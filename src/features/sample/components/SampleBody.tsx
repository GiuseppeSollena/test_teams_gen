...
const SampleBody: FC = () => {
    ...
    const openEditModal = useCallback((description: string, name :string) => {
      openModal(<SampleModal description={description} name={name} />)
    }, []);
                    ...
                    <tbody>
                        {data && data.length > 0 ? (
                            data.map((item: SampleTable) => (
                                <tr key={item.id} className={styles.tableRow} onClick={() => openEditModal(item.description, item.name)}>
                                    <td
